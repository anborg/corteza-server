...
func (h *AuthHandlers) sessionsProc(req *request.AuthReq) error {
	...
	case len(req.Request.PostFormValue("deleteSession")) > 0:
		for _, s := range ss {
			if s.ID != req.Request.PostFormValue("deleteSession") {
				continue
			}
	...
	req.RedirectTo = templates.GetLinks().Sessions
	return nil
}
	...
	if set, err = h.SessionManager.Search(req.Context(), req.User.ID); err != nil {
		...
		for i := range set {
			...
			ss[i] = userSession{
				...
				SameRemoteAddr: !isCurrent && set[i].RemoteAddr == req.Request.RemoteAddr,
				UserAgent:      set[i].UserAgent,
				SameUserAgent:  !isCurrent && set[i].UserAgent == req.Request.UserAgent(),
			}
		}
	}
