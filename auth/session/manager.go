...
func NewManager(store store.AuthSessions, opt options.AuthOpt, log *zap.Logger) *Manager {
	...
	m.cstore = store
	m.sstore = CortezaSessionStore(store, []byte(opt.Secret))
	return m
}
...
func (m Manager) Get(r *http.Request) *sessions.Session {
	...
	ses.Options.MaxAge = int(m.opt.SessionLifetime / time.Second)
	ses.Options.Secure = m.opt.SessionSecureCookies
	return ses
}

func (m *Manager) Save(w http.ResponseWriter, r *http.Request) {
	if err := m.Get(r).Save(r, w); err != nil {
		m.log.Warn("failed to save temporary session", zap.Error(err))
	}
}
