...
func init() {
	gob.Register(keyOriginalSession)
	gob.Register(map[string]string{})
}

func CortezaSessionStore(store store.AuthSessions, keyPairs ...[]byte) *cortezaSessionStore {
...
func (s cortezaSessionStore) load(ctx context.Context, session *sessions.Session) error {
	cortezaSession, err := s.store.LookupAuthSessionByID(ctx, session.ID)
	if err != nil {
		return err
	}

	if cortezaSession.ExpiresAt.Before(*now()) {
		return fmt.Errorf("session expired")
	}

	if err = gob.NewDecoder(bytes.NewReader(cortezaSession.Data)).Decode(&session.Values); err != nil {
		return fmt.Errorf("failed to decode session: %w", err)
	}

	// Store original session back for ref
	session.Values[keyOriginalSession] = cortezaSession
