...
func authSettingsAutoDiscovery(ctx context.Context, log *zap.Logger, svc settingsService) (err error) {
	...
	var (
		...
		isSecure = func() boolWrapper {
			return func() bool {
				_, has := os.LookupEnv("LETSENCRYPT_HOST")
				return has
			}
		}
	...
	list := []struct {
		...
		{
			"auth.federated.enabled",
			"PROVISION_SETTINGS_AUTH_FEDERATED_ENABLED",
			wrapBool(true),
			false},

		{
			"auth.federated.session-store-secret",
			"PROVISION_SETTINGS_AUTH_FEDERATED_SESSION_STORE_SECRET",
			rand,
			true},

		{
			"auth.federated.session-store-secure",
			"PROVISION_SETTINGS_AUTH_FEDERATED_SESSION_STORE_SECURE",
			isSecure(),
			false},
