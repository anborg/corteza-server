...
func (u *user) Encode(ctx context.Context, doc *Document, state *envoy.ResourceState) (err error) {
	...
	// Encode timestamps
	u.ts, err = resource.NewTimestampEncoder().
		WithTemplate(u.encoderConfig.TimeLayout).
		WithTimezone(u.encoderConfig.Timezone).
		EncodeCUDA(u.res.CreatedAt, u.res.UpdatedAt, u.res.DeletedAt, nil).
		Encode("suspendedAt", u.res.SuspendedAt).
		End()
	if err != nil {
		return err
	}
