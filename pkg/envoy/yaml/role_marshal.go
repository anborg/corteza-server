...
func (r *role) Encode(ctx context.Context, doc *Document, state *envoy.ResourceState) (err error) {
	...
	r.ts, err = resource.NewTimestampEncoder().
		WithTemplate(r.encoderConfig.TimeLayout).
		WithTimezone(r.encoderConfig.Timezone).
		EncodeCUDA(r.res.CreatedAt, r.res.UpdatedAt, r.res.DeletedAt, r.res.ArchivedAt).
		End()
