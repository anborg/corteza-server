...
func (n *composeModule) Encode(ctx context.Context, doc *Document, state *envoy.ResourceState) (err error) {
	...
	// Timestaps
	n.ts, err = resource.NewTimestampEncoder().
		WithTemplate(n.encoderConfig.TimeLayout).
		WithTimezone(n.encoderConfig.Timezone).
		EncodeCUDA(n.res.CreatedAt, n.res.UpdatedAt, n.res.DeletedAt, nil).
		End()
	if err != nil {
		return err
	}
	...
	for _, f := range n.fields {
		// Timestaps
		f.ts, err = resource.NewTimestampEncoder().
			WithTemplate(n.encoderConfig.TimeLayout).
			WithTimezone(n.encoderConfig.Timezone).
			EncodeCUDA(f.res.CreatedAt, f.res.UpdatedAt, f.res.DeletedAt, nil).
			End()
		if err != nil {
			return err
		}
