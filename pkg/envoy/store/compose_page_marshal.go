...
func (n *composePage) Prepare(ctx context.Context, s store.Storer, state *envoy.ResourceState) (err error) {
	// Get related namespace
	n.relNS, err = findComposeNamespaceRS(ctx, s, state.ParentResources, n.res.RefNs.Identifiers)
	if err != nil {
		return err
	}
	if n.relNS == nil {
	...
	if n.res.RefMod != nil {
		n.relMod, err = findComposeModuleRS(ctx, s, n.relNS.ID, state.ParentResources, n.res.RefMod.Identifiers)
		if err != nil {
			return err
		}
		if n.relMod == nil {
	...
	if n.res.RefParent != nil {
		n.relParent, err = findComposePageRS(ctx, s, n.relNS.ID, state.ParentResources, n.res.RefParent.Identifiers)
		if err != nil {
			return err
		}
		if n.relParent == nil {
	...
	for _, refChart := range n.res.RefCharts {
		chr, err := findComposeChartRS(ctx, s, n.relNS.ID, state.ParentResources, refChart.Identifiers)
		if err != nil {
			return err
		}
		if chr == nil {
...
// should be changed.
func (n *composePage) Encode(ctx context.Context, s store.Storer, state *envoy.ResourceState) (err error) {
	res := n.res.Res
	...
	if res.NamespaceID <= 0 {
		ns := resource.FindComposeNamespace(state.ParentResources, n.res.RefNs.Identifiers)
	...
	if n.res.RefMod != nil {
		...
		if res.ModuleID <= 0 {
			mod := resource.FindComposeModule(state.ParentResources, n.res.RefMod.Identifiers)
			res.ModuleID = mod.ID
		}
	}
	...
	if n.res.RefParent != nil {
		...
		if res.SelfID <= 0 {
			mod := resource.FindComposePage(state.ParentResources, n.res.RefParent.Identifiers)
			res.SelfID = mod.ID
		}
	}
	...
	getModID := func(id string) uint64 {
		mod := n.relMods[id]
		if mod == nil || mod.ID <= 0 {
			mod = resource.FindComposeModule(state.ParentResources, resource.MakeIdentifiers(id))
			if mod == nil || mod.ID <= 0 {
				return 0
			}
		}
