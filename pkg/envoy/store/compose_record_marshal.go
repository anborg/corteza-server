...
func (n *composeRecord) Prepare(ctx context.Context, s store.Storer, state *envoy.ResourceState) (err error) {
	...
	n.missing = true
	n.relMod = resource.FindComposeModule(state.ParentResources, n.res.RefMod.Identifiers)
	if n.relMod == nil && n.relNS.ID > 0 {
		n.relMod, err = findComposeModuleS(ctx, s, n.relNS.ID, makeGenericFilter(n.res.RefMod.Identifiers))
		if err != nil {
			return err
		}
		if n.relMod != nil {
...
func (n *composeRecord) Encode(ctx context.Context, s store.Storer, state *envoy.ResourceState) (err error) {
	// Namespace
	nsID := n.relNS.ID
	if nsID <= 0 {
	...
	mod := n.relMod
	if mod.ID <= 0 {
		m := resource.FindComposeModule(state.ParentResources, n.res.RefMod.Identifiers)
		mod.ID = m.ID
	}
	...
	// Firstly the users provided by the record state itself
	...
	for _, ref := range state.ParentResources {
