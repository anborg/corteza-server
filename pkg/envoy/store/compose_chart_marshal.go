...
func (n *composeChart) Prepare(ctx context.Context, s store.Storer, state *envoy.ResourceState) (err error) {
	...
	for i, rMod := range n.res.RefMods {
		n.relMods[i], err = findComposeModuleRS(ctx, s, n.relNS.ID, state.ParentResources, rMod.Identifiers)
		if err != nil {
			return err
		}
		if n.relMods[i] == nil {
...
// should be changed.
func (n *composeChart) Encode(ctx context.Context, s store.Storer, state *envoy.ResourceState) (err error) {
	res := n.res.Res
	...
	for i, r := range res.Config.Reports {
		relMod := n.relMods[i]
		if relMod == nil {
			relMod = resource.FindComposeModule(state.ParentResources, n.res.RefMods[i].Identifiers)
		}
