...
func (n *rbacRule) Prepare(ctx context.Context, s store.Storer, state *envoy.ResourceState) (err error) {
	// Init global state
	if gRbacRules == nil {
...
// should be changed.
func (n *rbacRule) Encode(ctx context.Context, s store.Storer, state *envoy.ResourceState) (err error) {
	res := n.res.Res

	// Related role
	res.RoleID = n.relRole.ID
	if res.RoleID <= 0 {
		rl := resource.FindRole(state.ParentResources, n.res.RefRole.Identifiers)
		res.RoleID = rl.ID
	}
	if res.RoleID == 0 {
		return resource.RoleErrUnresolved(n.res.RefRole.Identifiers)
	...
	if _, exists := gRbacRules[rbacRuleIndex(res)]; !exists {
		return store.CreateRbacRule(ctx, s, res)
	}
	...
	case resource.Replace,
		resource.MergeRight:
		return store.UpdateRbacRule(ctx, s, res)
	}
