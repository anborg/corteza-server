...
func (n *composeModule) Prepare(ctx context.Context, s store.Storer, state *envoy.ResourceState) (err error) {
	...
	for _, refMod := range n.res.RefMods {
		...
		if n.relNS.ID > 0 {
			mod, err = findComposeModuleS(ctx, s, n.relNS.ID, makeGenericFilter(refMod.Identifiers))
			if err != nil {
				return err
			}
		}
		if mod == nil {
			mod = resource.FindComposeModule(state.ParentResources, refMod.Identifiers)
		}
		if mod == nil {
...
// should be changed.
func (n *composeModule) Encode(ctx context.Context, s store.Storer, state *envoy.ResourceState) (err error) {
	res := n.res.Res
	...
	for i, f := range res.Fields {
		...
		if f.Kind == "Record" {
			...
			if modID <= 0 {
				...
				mod := resource.FindComposeModule(state.ParentResources, ii)
				if mod == nil || mod.ID <= 0 {
