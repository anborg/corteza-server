...
func (n *role) Prepare(ctx context.Context, s store.Storer, state *envoy.ResourceState) (err error) {
...
// should be changed.
func (n *role) Encode(ctx context.Context, s store.Storer, state *envoy.ResourceState) (err error) {
	res := n.res.Res
	...
	// Create a fresh role
	if !exists {
		return store.CreateRole(ctx, s, res)
	}
	...
	err = store.UpdateRole(ctx, s, res)
	if err != nil {
		return err
	}
